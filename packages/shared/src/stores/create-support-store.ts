...
// Shared passenger/driver support store. The two apps' logic is identical; only
// the default participant type ('passenger' vs 'driver') differs, so it is
// injected. Role-specific subject lists stay in each app's types module.
export function createSupportStore(defaultParticipant: SupportParticipantType) {
  return defineStore('support', () => {
    ...
    let participant: SupportParticipantType = defaultParticipant
    ...
    async function loadRooms(participantType: SupportParticipantType = defaultParticipant) {
