...
import { shareReferral } from '@edtaxi/shared/composables/telegram/referral'
...
useAutoRefresh(async () => {
  const [me, promos] = await Promise.all([getBonusOverview(), getMyPromotions('driver')])
  overview.value = me
...
// «Поделиться» открывает нативный шит Telegram (выбор чата) с диплинком на
// бота: друг открывает ссылку, мини-апп запускается сам, и после входа бонусы
// начисляются автоматически — вводить код руками больше не нужно.
function shareCode() {
  if (!overview.value)
    return
...
async function joinPromo(promo: BonusPromotion) {
  ...
  try {
    ...
    const promos = await getMyPromotions('driver')
    promotions.value = promos.promotions
  }
  catch (error) {
        ...
        <section class="mt-6">
          ...
          <div v-else class="mt-3 space-y-3">
            <article v-for="promo in promotions" :key="promo.id" class="rounded-3xl app-card p-4">
              ...
              <p v-if="promo.message" class="mt-1 text-xs text-slate-300 leading-4 light:text-slate-600">
                {{ promo.message }}
              </p>
