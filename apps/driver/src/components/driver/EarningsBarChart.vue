...
import type { EarningsBucket } from '~/utils/earningsChart'
import { formatCompactNumber, formatTenge, formatTripCount, niceCeil } from '~/utils/earningsChart'
...
const maxValue = computed(() => niceCeil(Math.max(...props.buckets.map(bucket => bucket.total), 0)))
...
const bars = computed<BarGeometry[]>(() => {
  ...
  // Подписи оси X прореживаются на плотном окне (14 дней), якорь — последняя корзина.
  const labelStep = count > 12 ? 2 : 1

  return props.buckets.map((bucket, index) => {
    const bandX = bandWidth * index
    const x = bandX + (bandWidth - barWidth) / 2
    let top = PLOT_BOTTOM - (bucket.total / maxValue.value) * (PLOT_BOTTOM - PLOT_TOP)
    if (bucket.total > 0)
    ...
    return {
      ...
      index,
      path: bucket.total > 0 ? roundedTopBarPath(x, barWidth, top, PLOT_BOTTOM) : '',
...
const gridlines = computed(() => [
  { label: formatCompactNumber(maxValue.value), y: PLOT_TOP },
  { label: formatCompactNumber(maxValue.value / 2), y: (PLOT_TOP + PLOT_BOTTOM) / 2 },
])
...
const selectedLabelY = computed(() => {
  ...
  const top = PLOT_BOTTOM - (bar.bucket.total / maxValue.value) * (PLOT_BOTTOM - PLOT_TOP)
  ...
  >
    <svg v-if="bars.length" aria-hidden="true" class="block" :height="HEIGHT" :width="width">
      ...
      <g v-for="line in gridlines" :key="line.y">
        ...
        <text class="fill-slate-500 text-[10px] font-700" text-anchor="end" :x="width - 2" :y="line.y - 4">
          {{ line.label }}
        </text>
      </g>
      ...
      <g v-for="bar in bars" :key="bar.bucket.key" class="group cursor-pointer">
        <path
          ...
          :class="bar.index === selectedIndex ? 'fill-main-500' : 'fill-main-700 group-hover:fill-main-600'"
          :d="bar.path"
        />
        <text
          v-if="bar.showLabel"
          class="text-[10px] font-700 transition-colors duration-150"
          :class="bar.index === selectedIndex ? 'fill-slate-200' : 'fill-slate-500'"
          text-anchor="middle"
          :x="bar.centerX"
          :y="X_LABEL_Y"
        >
      ...
      <text
        ...
        :y="selectedLabelY"
      >
        {{ selectedBar.bucket.total > 0 ? formatCompactNumber(selectedBar.bucket.total) : '0' }}
      </text>
    </svg>
    ...
    <table class="sr-only">
      ...
      <tbody>
        <tr v-for="bucket in buckets" :key="bucket.key">
          ...
          <td>{{ formatTenge(bucket.total) }}</td>
          <td>{{ formatTripCount(bucket.tripCount) }}</td>
        </tr>
      </tbody>
    </table>
  </div>
