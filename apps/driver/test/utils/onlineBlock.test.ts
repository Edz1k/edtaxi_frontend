...
describe('onlineBlockTargetFor', () => {
  ...
  it('долг по наличным превысил порог → экран заработка (пополнение)', () => {
    expect(onlineBlockTargetFor('Долг по наличным поездкам превысил 1000 ₸. Пополните баланс, чтобы выйти на линию.'))
      .toBe('debt')
  })

  // Единственная причина, которая закрывается вне апы — в чате с ботом.
  it('нет трансляции геопозиции → чат с ботом', () => {
    expect(onlineBlockTargetFor('Включите трансляцию геопозиции в чате с ботом, чтобы выйти на линию'))
      .toBe('live-location')
  })

  // Своё сообщение map.vue про выключенную геолокацию телефона цель ставит
  // явно и сюда не попадает, но похожие формулировки не должны уводить в бота.
  it('выключенная геолокация телефона трансляцией не считается', () => {
    expect(onlineBlockTargetFor('Включите геолокацию, чтобы выйти на линию.')).toBe('verification')
  })
