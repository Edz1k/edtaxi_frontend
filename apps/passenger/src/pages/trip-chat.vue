...
useHead({
  title: () => `${t('tripChat.title')} | Telegram Taxi`,
})
...
const driverName = computed(() => activeTrip.value?.driver?.name || t('tripChat.driver'))

onMounted(async () => {
  await trips.refreshActiveTrip().catch(() => {})
  if (!trips.activeTrip) {
    router.replace('/map')
    return
  }
  notifications.connect()
  await tripChat.openChat(trips.activeTrip.id)
  scrollToBottom()
})
  ...
  <main class="tg-safe-x tg-safe-screen h-full flex flex-col app-screen pb-[calc(var(--app-safe-area-bottom)+1rem)] pt-[calc(var(--app-safe-area-top)+0.75rem)] text-white">
    <section class="mx-auto max-w-sm min-h-0 w-full flex flex-1 flex-col">
      ...
      <div class="mb-3 flex shrink-0 items-center gap-2">
        <button
          :aria-label="t('tripChat.backAria')"
